...
Turn any product into a **donation / fundraising widget** by giving it a **goal** (`goalAmount`). When a product has a goal, the widget automatically renders a **progress bar** showing how much has been raised versus the goal, plus a **Donate** button — no extra configuration needed.

![Donation widget — progress card with goal, raised amount, and Donate Now button](../assets/images/donation-widget-card.png)

![Donation widget — amount entry modal with orange description and payment preview](../assets/images/donation-widget-modal.png)

**Visual style (matches the Home screen):**

- Typography: **DM Sans** — product description as a label above the amounts, bold raised amount, larger modal title
- Colors: text `#1a1a1a` / `#6b6b6b`, accent **orange** `#ff7f41` on raised amount, progress percentage, description, and CTA
- Card: white background, rounded corners, progress bar, and orange pill **Donate Now** button
- Modal: orange product description (e.g. "Support our cause"), orange **USDC** badge, and payment preview
- "Raised" is computed automatically from confirmed (paid) payments generated by the widget
- Combine with `allowCustomAmount: true` so donors can enter any amount (**required** when `goalAmount` is set — the widget prompts for the amount before checkout)

### Create a donation widget (bundle)

```javascript
await fetch('https://api.achylo.com/api/v1/widgets/bundle', {
  method: 'POST',
  headers: { 'X-API-Key': 'YOUR_API_KEY', 'Content-Type': 'application/json' },
  body: JSON.stringify({
    resourceType: 'product',
    productDetails: {
      name: 'Help us reach our goal',
      description: 'Support the campaign',
      allowCustomAmount: true,   // donors choose the amount
      goalAmount: '80000'        // 👈 fundraising goal in USDC → enables the progress bar
    },
    originWhitelist: ['https://yourcause.org'],
    payoutAddress: '0x...SMART_ACCOUNT_ADDRESS'
  })
});
```

> `goalAmount` is optional. If omitted (or `null`), the widget behaves as a normal payment button with no progress bar. When set, **`allowCustomAmount` must be `true`** — enforced in the dashboard and API. You can also set/clear the goal later via `PUT /api/v1/products/:id`.

### Dashboard

Set **"Fundraising goal (USDC)"** when creating a product or a Widget v2 bundle:

- **Stats → Payments → Widgets & Products** (recommended)
- **User Profile → Payment Widgets → Create Widget V2** (Step 3: Product Details)

When `goalAmount` is set, the embed shows the donation progress bar automatically.
...
```json
{
  ...
  "product": {
    ...
    "goalAmount": "80000",
    "isDonation": true,
    "raisedAmount": "23505.00",
    "progressPercent": 29.4
  }
}
```

The SDK reads these to render the bar (`progressPercent`), the amounts (`raisedAmount` / `goalAmount`), and refreshes after a successful donation.
...
- ✅ Check `widget.js?v=1.7` loaded in console
