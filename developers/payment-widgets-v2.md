...
- Combine with `allowCustomAmount: true` so donors can enter any amount
...
> `goalAmount` is optional. If omitted (or `null`), the widget behaves as a normal payment button with no progress bar. You can also set/clear the goal later via `PUT /api/v1/products/:id`.
